...
if __name__ == "__main__" and _enabled("Blocked"):
    run_gemm_sweep(
        "Blocked",
        lambda A, B, C, bm, bn, bk, nw: gemm0.blocked_matmul(A, B, C, bm, bn, bk, nw),
    )
...
# ------------------------------------------------------------
# gemm-2: persistent_matmul
# signature: persistent_matmul(A, B, C, BLOCK_M, BLOCK_N, BLOCK_K, num_buffers, num_warps, SchedulerImpl)
# ------------------------------------------------------------
if __name__ == "__main__" and _enabled("persistent"):
    NUM_BUFFERS = 4
    SchedulerImpl = gemm2.PersistentTileScheduler  # pass the CLASS

    def _persistent_runner(A, B, C, bm, bn, bk, nw):
        gemm2.persistent_matmul(A, B, C, bm, bn, bk, NUM_BUFFERS, nw, SchedulerImpl)
