...
if __name__ == "__main__":
    ...
    # AOT compile: specify ABI (pointers + i32 + constexpr)
    src = triton.compiler.ASTSource(
        fn=blocked_matmul_kernel,
        signature={
            "a_desc": "*fp16",    # change to "*f32"/"*bf16" if needed
            "b_desc": "*fp16",
            # NOTE: if you flip FP8_OUTPUT=True (constexpr), switch c_ptr to "*i8"
            "c_desc": "*fp16",
            "TRANSPOSE_B": "constexpr",
            "num_warps": "constexpr",
        },
        constexprs={
            "BLOCK_SIZE_M": 128,
            ...
            "BLOCK_SIZE_K": 64,
            "GROUP_SIZE_M": 8,
            "FP8_OUTPUT": False,     # if True, also change c_ptr in signature to "*i8"
            "WARP_SPECIALIZE": True,
        },
        attrs={"num_warps": 4, "num_stages": 2},
    )

    # Hopper target: ("cuda", sm=90, warp_size=32)
    target = GPUTarget("cuda", 100, 32)

    # Compile to PTX/CUBIN/TTIR/etc.; 'output.asm' contains multiple blobs
    output = triton.compile(src, target=target)

    # Save PTX for inspection
    Path("matmul_tma.ptx").write_text(output.asm["ptx"])
    print("AOT compiled:", output.name, "-> matmul_tma.ptx")